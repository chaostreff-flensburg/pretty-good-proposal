...
onMounted(async () => {
    ...
    try {
        ...
        proposal.value.comments_count = proposal.value.opinions.filter((opinion) => !!opinion.comment && opinion.comment !== '').length
        proposal.value.vote_count = proposal.value.opinions.filter((opinion) => !!opinion.vote).length
        const { data: session } = await supabase.auth.getSession()
        currentUserId.value = session.session?.user?.id
        myOpinion.value = data[0].opinions.filter(opinion => opinion.profile_id === currentUserId.value)[0]
    }
    catch (error) {
        console.error(error)
        toast.add({
                ...
                <Card>
                    ...
                    <template #content>
                        <article v-for="(value, name, index) in proposal.proposal">
                            <span class="font-bold">{{ proposal_fields[name]?.de || name }}: </span>
                            <span>{{ value }}</span>
                        </article>
                    </template>
                </Card>
            ...
            <div class="col">
                <div class="flex">
                    ...
                    <Card style="max-width: 240px; max-height: 120px;">
                        <template #title>Status: {{ status[proposal.status]?.name || proposal.status }} </template>
                    </Card>
                </div>
                <h3>Meinungen</h3>
                <Card>
                    ...
                    <template #content>
                        <OpinionForm v-if="proposalId" :proposalId="route.params.id" :myOpinion="myOpinion"
                            :currentUserId="currentUserId" />
                    </template>
                </Card>

                <h4>Alle Meinungen</h4>
                <article v-if="proposal.opinions?.length > 0" v-for="opinion in proposal.opinions">
                    <Card>
                        <template #title>{{ opinion.special_vote || opinion.vote }} {{ opinion.profiles.username }}
                        </template>
                ...
                <h3>Status Ändern</h3>
                <ProposalStatusForm v-if="proposalId" :proposalId="route.params.id" :proposalStatus="proposal.status"/>
            </div>
        </div>
    </template>
</template>